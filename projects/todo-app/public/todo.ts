...
const inputData = document.getElementById("add-input") as HTMLInputElement; // input 창
...
// 상태 변수들
const todoListArray:{id:string, itemText:string}[] = []; // todoList 배열
...
if(todoList){
    todoList.addEventListener("click",(event)=>{
        ...
        if(target.closest(".edit-btn")){
            const item = target.closest(".todo-item");
            const titleSpan = item?.querySelector(".todo-title")as HTMLSpanElement;
            if(item && titleSpan){
                ...
                const saveEdit = ()=>{ // 새로운 input 창에서 엔터치면 해당 배열에 item 값 변경
